...
```
mysql
mysql -h 120.76.214.138 -P 3336 -u datauser -pfgecctv714 -D cloudringproperty
mysql -h 192.168.1.200 -P 3306 -udatauser -p123456 
mysql -h 192.168.1.200 -P 3306 -u datauser -p
```
...
授权：
GRANT SELECT, INSERT , UPDATE, DELETE ON *.* TO 'pig'@'%'
GRANT ALL ON *.* TO 'datauser'@'%';
INSERT INTO user(host, user, ssl_cipher,x509_issuer,x509_subject) values('localhost', 'datauser', '','','');
...
```
[root@localhost ~]# vim /etc/sysconfig/network-scripts/ifcfg-enp2s0
TYPE=Ethernet
